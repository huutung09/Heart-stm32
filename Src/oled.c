...
void HAL_I2C_MemTxCpltCallback(I2C_HandleTypeDef *hi2c)
{
    ...
    {
        // OLED_WR_Byte((x & 0x0f), OLED_CMD); //0+dia chi thap
        dat = 0x00;
    ...
    {
        // OLED_WR_Byte(((x & 0xf0) >> 4) | 0x10, OLED_CMD); //8+dia chi cao
        dat = 0x10;
        HAL_I2C_Mem_Write_IT(&hi2c1, OLED_ADDR, 0x00, I2C_MEMADD_SIZE_8BIT, &dat, 1);
        whatToDo = 3;
    }
    ...
    {
        // OLED_WR_Byte(0xb0 + y, OLED_CMD); // b+dia chi trang
        dat = 0xb0 + y;
        HAL_I2C_Mem_Write_IT(&hi2c1, OLED_ADDR, 0x00, I2C_MEMADD_SIZE_8BIT, &dat, 1);
        whatToDo = 2;
    }
...
//Hien thi chuoi tai mot vi tri xac dinh
//x:0~127
//y:0~7
...
{
    OLED_WR_Byte(0xAE, OLED_CMD); //--display off
    OLED_WR_Byte(0x00, OLED_CMD); //---4 bit dia chi thap
    OLED_WR_Byte(0x10, OLED_CMD); //---4 bit dia chi cao
    OLED_WR_Byte(0x40, OLED_CMD); //--set start line address
    OLED_WR_Byte(0xB0, OLED_CMD); //-- dia chi trang
    OLED_WR_Byte(0x81, OLED_CMD); // contract control
    OLED_WR_Byte(0xf0, OLED_CMD); //--128
    OLED_WR_Byte(0xA1, OLED_CMD); //set segment remap
    OLED_WR_Byte(0xA6, OLED_CMD); //--a6 thuan ，a7 nguoc
    OLED_WR_Byte(0xA8, OLED_CMD); //--set multiplex ratio(1 to 64)
    OLED_WR_Byte(0x3F, OLED_CMD); //--1/32 duty
    OLED_WR_Byte(0xC8, OLED_CMD); //c0 hoac c8 lat doc
    ...
    OLED_WR_Byte(0xAF, OLED_CMD); //--AF la bat man hinh，AE la tat man hinh
    ...
    for (i = 0; i < 6; i++)
    {
		...
		//01000001
		//01000001
		const uint8_t c[] = {0x03, 0x03, 0x00, 0x3E, 0x41, 0x41, 0x41};
		OLED_Set_Pos(120, 4);
		for (i = 0; i < 7; i++)
    {
        OLED_WR_Byte(c[i], OLED_DATA);
    }
...
const uint8_t oled_nums_pos[] = {0, 11, 22, 36, 58, 69, 98, 109};
