...
// Use this to read the state to determine whether the conversion is complete
...
int16_t eachSampleDiff = 0; //How much is the difference from the previous sample
void max30102_cal()
{
    ...
    static uint8_t eachBeatSampleCount = 0;    //How many samples did this heartbeat go through?
    static uint8_t lastTenBeatSampleCount[10]; 
    static uint32_t last_iRed = 0;             
    uint8_t i, ii;
    for (i = 0; i < unreadSampleCount; i++)
    {
        if (sampleBuffTemp[i].red < 40000) //No fingers do not count, skip
        {
            heartRate = 0;
            spo2 = 0;
            eachSampleDiff = 0;
        ...
        eachSampleDiff = last_iRed - sampleBuffTemp[i].red;
        ...
        last_iRed = sampleBuffTemp[i].red;
        eachBeatSampleCount++;
    }
}
